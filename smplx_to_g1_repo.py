...
# adjust this fallback to actual GMR repo path
GMR_ROOT = "/home/leo/experiment/retarget/GMR"
GMR_ROOT = pathlib.Path(GMR_ROOT).resolve()
...
def process_file(smplx_file_path, tgt_file_path, tgt_robot, SMPLX_FOLDER, tgt_folder, total_files, verbose=False):
    ...
    for root, _, files in os.walk(tgt_folder):
        done += len([f for f in files if f.endswith('.npy')])
...
def main():
    ...
    for dirpath, _, filenames in os.walk(src_folder):
        for filename in natsorted(filenames):
            if filename.endswith("_stagei.npy"):
                continue
